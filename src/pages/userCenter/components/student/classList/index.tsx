...
import { Card, Row, Col, Space, Tag, Avatar, Modal, Form, Input, message } from 'antd'
...
import { IStudentClassInfo } from '@/services/userCenter/types'
...
const myClassList: FC = () => {
  ...
  const [classList, setClassList] = useState<IStudentClassInfo[]>([])
  const [searchText, setSearchText] = useState('')
  const [isSearch, setIsSearch] = useState(false)
  const dispatch = useDispatch()
  const [update, setUpdate] = useState(0)
  //通过搜索邀请码获取班级列表
  const { run: runGetStudentClassListBySearch } = useRequest(
    UserCenterServices.getStudentClassListByinvitePwd,
    {
      manual: true,
      onSuccess: (res) => {
  ...
  useEffect(() => {
    if (isSearch) {
      runGetStudentClassListBySearch({ invitePwd: searchText })
    } else {
  ...
  //通过搜索邀请码加入班级
  const handleJoinClassBySearch = (classInfo: any) => {
    runJoinClass({ classID: classInfo.ID, invitePwd: classInfo.invitePwd })
  }
            ...
            return (
              <Col key={`${classInfo.ID}${index}`} span={6}>
                <Card
                  ...
                  onClick={() => {
                    if (isSearch) {
                      handleJoinClassBySearch(classInfo)
                    } else {
